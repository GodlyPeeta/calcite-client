...
import calcite.client.event.events.ResolutionUpdateEvent
    ...
    init {
        shader = when {
            !OpenGlHelper.shadersSupported -> {
                calcite.client.CalciteMod.LOG.warn("Shaders are unsupported by OpenGL!")
                null
            }

            isIntegratedGraphics -> {
                calcite.client.CalciteMod.LOG.warn("Running on Intel Integrated Graphics!")
                null
            }
                ...
                try {
                    ...
                    calcite.client.CalciteMod.LOG.warn("Failed to load shaders")
                    e.printStackTrace()
