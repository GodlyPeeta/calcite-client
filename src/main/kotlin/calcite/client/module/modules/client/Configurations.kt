...
internal object Configurations : AbstractModule(
    ...
    init {
        BackgroundScope.launchLooping("Config Auto Saving", 60000L) {
            if (autoSaving && mc.currentScreen !is AbstractCalciteGui<*, *> && timer.tick(savingInterval.toLong())) {
                if (savingFeedBack) MessageSendHelper.sendChatMessage("Auto saving settings...")
                else calcite.client.CalciteMod.LOG.info("Auto saving settings...")
                ConfigUtils.saveAll()
            }
        }
    ...
    private fun updatePreset(setting: StringSetting, input: String, config: IConfig) {
        ...
        try {
            ...
            calcite.client.CalciteMod.LOG.warn("Couldn't set path!", e)
        ...
        fun copyPreset(name: String) {
            defaultScope.launch(Dispatchers.IO) {
                ...
                try {
                    ...
                    calcite.client.CalciteMod.LOG.error("Failed to copy preset", e)
                }
            }
        }
        ...
        fun deletePreset(name: String) {
            defaultScope.launch(Dispatchers.IO) {
                ...
                try {
                    ...
                    calcite.client.CalciteMod.LOG.error("Failed to delete preset", e)
                }
            }
        }
