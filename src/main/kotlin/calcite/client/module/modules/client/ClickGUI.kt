...
internal object ClickGUI : Module(
    ...
    val radius by setting("Radius", 0.0, 0.0..10.0, 0.2)
    ...
    init {
        ...
        sortBy.listeners.add { CalciteClickGui.reorderModules() }

        resetComponents.listeners.add {
            CalciteClickGui.windowList.forEach {
                it.resetPosition()
            }
        }
    }
    ...
    init {
        onEnable {
            if (mc.currentScreen !is CalciteClickGui) {
                HudEditor.disable()
                mc.displayGuiScreen(CalciteClickGui)
                CalciteClickGui.onDisplayed()
            }
        }
