...
internal object DiscordNotifs : Module(
    ...
    val avatar = setting("Avatar", "${calcite.client.CalciteMod.GITHUB_LINK}/assets/raw/assets/assets/icons/kamiGithub.png")

    private const val username = "${calcite.client.CalciteMod.NAME} ${calcite.client.CalciteMod.VERSION}"
            ...
            ConnectionUtils.runConnection(
                ...
                { connection ->
                    ...
                    if (response.isNotEmpty()) {
                        calcite.client.CalciteMod.LOG.info("Unexpected response from DiscordNotifs http request: $response")
                    }
                },
                {
                    calcite.client.CalciteMod.LOG.warn("Error while sending webhook", it)
                },
            )
        }
    }
