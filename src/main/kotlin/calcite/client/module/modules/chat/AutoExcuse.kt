...
internal object AutoExcuse : Module(
    ...
    private val file = File(calcite.client.CalciteMod.DIRECTORY + "excuses.txt")
            ...
            loadedExcuses = if (mode == Mode.EXTERNAL) {
                if (file.exists()) {
                    ...
                    try {
                        ...
                        calcite.client.CalciteMod.LOG.error("Failed loading excuses", e)
                    }
                    cacheList.toTypedArray()
                } else {
                    file.createNewFile()
                    MessageSendHelper.sendErrorMessage("$chatName Excuses file is empty!" +
                        ", please add them in the &7excuses.txt&f under the &7.minecraft/calcite&f directory.")
                    defaultExcuses
                }
            } else {
                defaultExcuses
