...
import calcite.client.util.EnchantmentUtils
import calcite.client.util.EntityUtils
import calcite.client.util.color.ColorGradient
import calcite.client.util.color.ColorHolder
import calcite.client.util.graphics.*
import calcite.client.util.graphics.font.*
import calcite.client.util.items.originalName
import calcite.client.util.math.Vec2d
import calcite.client.util.threads.safeListener
import calcite.commons.extension.ceilToInt
import calcite.commons.extension.floorToInt
...
internal object Nametags : Module(
    ...
    init {
        listener<RenderOverlayEvent> {
            ...
            val camPos = CalciteTessellator.camPos
            val vertexHelper = VertexHelper(GlStateUtils.useVbo())
            val xRange = 0..mc.displayWidth
            val yRange = 0..mc.displayHeight

            for ((entity, textComponent) in entityMap) {
                val pos = EntityUtils.getInterpolatedPos(entity, CalciteTessellator.pTicks()).add(0.0, (entity.height + yOffset.value).toDouble(), 0.0)
            ...
            for (itemGroup in itemMap) {
                val pos = itemGroup.getCenter(CalciteTessellator.pTicks()).add(0.0, yOffset.value.toDouble(), 0.0)
                val screenPos = ProjectionUtils.toScreenPos(pos)
                val dist = camPos.distanceTo(pos).toFloat() * 0.2f
                val distFactor = if (distScaleFactor.value == 0f) 1f else max(1f / (dist * distScaleFactor.value + 1f), minDistScale.value)
