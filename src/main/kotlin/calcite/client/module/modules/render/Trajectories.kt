...
import calcite.client.event.events.PlayerTravelEvent
...
import calcite.client.module.modules.player.FastUse
...
internal object Trajectories : Module(
    ...
    init {
        ...
        listener<RenderWorldEvent> {
            ...
            val buffer = CalciteTessellator.buffer
            glLineWidth(thickness.value)
            GlStateUtils.depth(false)
            CalciteTessellator.begin(GL_LINE_STRIP)
    ...
    private fun getPathOffset(): Vec3d {
        ...
        val eyePos = mc.player.getPositionEyes(CalciteTessellator.pTicks())
        val camPos = EntityUtils.getInterpolatedPos(mc.player, CalciteTessellator.pTicks()).add(ActiveRenderInfo.getCameraPosition())
    ...
    private class FlightPath(val throwingType: ThrowingType) {
        ...
        var position: Vec3d = mc.player.getPositionEyes(CalciteTessellator.pTicks())
            private set
        ...
        private fun getInterpolatedCharge() = prevItemUseCount.toDouble() + (mc.player.itemInUseCount.toDouble() - prevItemUseCount.toDouble()) * CalciteTessellator.pTicks().toDouble()
