package calcite.client.command

import kotlinx.coroutines.*
import calcite.client.AsyncLoader
import calcite.client.CalciteMod
import calcite.client.event.ClientExecuteEvent
...
import calcite.client.util.threads.onMainThread
import calcite.command.AbstractCommandManager
import calcite.command.Command
import calcite.command.CommandBuilder
import calcite.command.utils.CommandNotFoundException
import calcite.command.utils.SubCommandNotFoundException
...
object CommandManager : AbstractCommandManager<ClientExecuteEvent>(),
    calcite.client.AsyncLoader<List<Class<out ClientCommand>>> {
    ...
    override fun preLoad0(): List<Class<out ClientCommand>> {
        ...
        val list = ClassUtils.findClasses<ClientCommand>("calcite.client.command.commands")

        val time = stopTimer.stop()

        calcite.client.CalciteMod.LOG.info("${list.size} commands found, took ${time}ms")
        return list
    }
    ...
    override fun load0(input: List<Class<out ClientCommand>>) {
        ...
        val time = stopTimer.stop()
        calcite.client.CalciteMod.LOG.info("${input.size} commands loaded, took ${time}ms")
    }
    ...
    fun runCommand(string: String) {
        defaultScope.launch {
            ...
            calcite.client.CalciteMod.LOG.debug("Running command with args: [${args.joinToString()}]")
            ...
            try {
                ...
                calcite.client.CalciteMod.LOG.warn("Error occurred while running command!", e)
            }
        }
    }
