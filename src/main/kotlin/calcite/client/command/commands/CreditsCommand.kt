        ...
        executeAsync {
            val contributors = getContributors() ?: run {
                MessageSendHelper.sendErrorMessage("Failed to retrieve contributors from Github API.\n" +
                    "Checkout the page manually: &9${calcite.client.CalciteMod.GITHUB_LINK}/client/graphs/contributors")
                return@executeAsync
            }
    ...
    private fun getContributors(): Array<GithubUser>? {
        return try {
            val rawJson = ConnectionUtils.requestRawJsonFrom(url) {
                calcite.client.CalciteMod.LOG.error("Failed to load Github contributors", it)
            }
            gson.getAdapter(Array<GithubUser>::class.java).fromJson(rawJson)
        } catch (e: Exception) {
            calcite.client.CalciteMod.LOG.error("Failed to parse Github contributors", e)
            null
        }
    }
