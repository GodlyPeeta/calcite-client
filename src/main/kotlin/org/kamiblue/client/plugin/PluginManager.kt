...
internal object PluginManager : AsyncLoader<List<PluginLoader>> {
    ...
    private val kamiVersion = DefaultArtifactVersion(CalciteMod.VERSION_MAJOR)
    ...
    fun loadAll(loaders: List<PluginLoader>) {
        ...
        CalciteMod.LOG.info("Loaded ${loadedPlugins.size} plugins!")
    }
    ...
    private fun checkPluginLoaders(loaders: List<PluginLoader>): List<PluginLoader> {
        ...
        for (loader in loaders) {
            ...
            if (CalciteMod.ready && !loader.info.hotReload) {
                invalids.add(loader)
            }
    ...
    fun load(loader: PluginLoader) {
        synchronized(this) {
            val hotReload = CalciteMod.ready && !loader.info.hotReload
    ...
    private fun loadWithoutCheck(loader: PluginLoader) {
            ...
            val plugin = runCatching(loader::load).getOrElse {
                when (it) {
                    is ClassNotFoundException -> {
                        CalciteMod.LOG.warn("Main class not found in plugin $loader", it)
                    }
                    is IllegalAccessException -> {
                        CalciteMod.LOG.warn(it.message, it)
                    }
                    else -> {
                        CalciteMod.LOG.error("Failed to load plugin $loader", it)
                    }
                }
                return
            }
        ...
        CalciteMod.LOG.info("Loaded plugin ${plugin.name}")
    }

    fun unloadAll() {
        loadedPlugins.filter { it.hotReload }.forEach(PluginManager::unloadWithoutCheck)

        CalciteMod.LOG.info("Unloaded all plugins!")
    }
    ...
    private fun unloadWithoutCheck(plugin: Plugin) {
        ...
        CalciteMod.LOG.info("Unloaded plugin ${plugin.name}")
    }
