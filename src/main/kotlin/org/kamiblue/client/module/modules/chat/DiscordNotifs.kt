...
internal object DiscordNotifs : Module(
    ...
    val avatar = setting("Avatar", "${CalciteMod.GITHUB_LINK}/assets/raw/assets/assets/icons/kamiGithub.png")

    private const val username = "${CalciteMod.NAME} ${CalciteMod.VERSION}"
            ...
            ConnectionUtils.runConnection(
                ...
                { connection ->
                    ...
                    if (response.isNotEmpty()) {
                        CalciteMod.LOG.info("Unexpected response from DiscordNotifs http request: $response")
                    }
                },
                {
                    CalciteMod.LOG.warn("Error while sending webhook", it)
                },
            )
        }
    }
