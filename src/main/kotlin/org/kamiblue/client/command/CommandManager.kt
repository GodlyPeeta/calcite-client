...
object CommandManager : AbstractCommandManager<ClientExecuteEvent>(), AsyncLoader<List<Class<out ClientCommand>>> {
    ...
    override fun preLoad0(): List<Class<out ClientCommand>> {
        ...
        CalciteMod.LOG.info("${list.size} commands found, took ${time}ms")
        return list
    }
    ...
    override fun load0(input: List<Class<out ClientCommand>>) {
        ...
        val time = stopTimer.stop()
        CalciteMod.LOG.info("${input.size} commands loaded, took ${time}ms")
    }
    ...
    fun runCommand(string: String) {
        defaultScope.launch {
            ...
            CalciteMod.LOG.debug("Running command with args: [${args.joinToString()}]")
            ...
            try {
                ...
                CalciteMod.LOG.warn("Error occurred while running command!", e)
            }
        }
    }
